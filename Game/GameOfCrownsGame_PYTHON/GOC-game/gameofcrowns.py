...
#i defined some colors
BLACK    = (  0,  0,  0)
WHITE    = ( 255, 255, 255)
LIGHTBLUE    = (  133, 214,  255)

#i defined dimensions
WIN_WIDTH = 800
WIN_HEIGHT = 550
...
#i created camera class
class Camera(object):
    def __init__(self, camera_func, width, height): #i called the camera fucntion and make the sprite a rectangular camera
        self.camera_func = camera_func
        self.state = pygame.Rect(0, 0, width, height)#left, top, width,height
...
class Character(Thing):
    def __init__(self, x, y):
        ...
        self.onGround = False
        self.image = pygame.image.load('jkm.png')
        #self.image = pygame.transform.scale('3d small people.jpg', (50, 50))
        #self.image = pygame.Surface((25,25))
        #self.image.fill(WHITE)
        self.image.convert()
        self.rect = pygame.Rect(x, y, 35, 43)
    ...
    def collide(self, xspeed, yspeed, platforms):
        for p in platforms: #for each regular platform
            if pygame.sprite.collide_rect(self, p):#when character collides with platform , block character's path:
                #if moving right,right side of character sprite is
                # equivalent to the left side of the platform
                if xspeed > 0:
...
#I created platform class
class Platform(Thing):
    def __init__(self, x, y):
        #I used Thing class to intialize platform sprite as a separate entity
        Thing.__init__(self)
        #i set rectangle sprites to the screen
        self.image = pygame.Surface((25, 25))
        self.image.convert()
...
class coinSprite(Platform):
    def __init__(self, x, y): #I used platform class to intialize coin sprite as apart of the platform entity
        Platform.__init__(self, x, y)
        #set and load image for coinsprite to the screen
        self.image = pygame.Surface((25, 25))
        self.image = pygame.image.load("coin.png")
        self.image = self.image.convert()
        #i let coinsprite have properties of a rectangle
